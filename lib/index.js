...
import { hostMatchesList, normalizeProxyServer, shouldProxy } from './pure.js';
...
/**
 * Wrapper-level noProxy decision — thin shell over pure.js. The proxied
 * agent is scheme-agnostic (raw ProxyAgent), so bypass matching lives in
 * hostMatchesList and the routing decision in shouldProxy.
 */
function noProxyMatch(opts, noProxy) {
	return hostMatchesList(opts, noProxy);
}
