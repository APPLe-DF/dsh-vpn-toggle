...
import { normalizeProxyServer, shouldProxy } from './pure.js';
...
function defaultState() {
	return { enabled: false, mode: 'all', proxy: '', noProxy: DEFAULT_NO_PROXY, allowProxy: '' };
}
...
/**
 * Enable-path precheck: probe the configured proxy and return a warning
 * string when the port is dead. Never blocks the enable — unusual setups
 * (e.g. proxy that only listens on demand) must not be locked out.
 */
async function precheckProxyForEnable() {
	const st = readState();
	if (!st.proxy) return '';
	return (await probeProxyAlive(st.proxy)) ? '' : '代理端口无响应';
}
...
function notifyState(st, warning) {
	try {
		...
		new Notification({
			title: 'DSH VPN 开关',
			body: st.enabled ? `已启用 → ${st.proxy}${suffix}` : `已关闭（直连）${suffix}`,
			silent: true
		}).show();
	} catch {}
}
...
async function handleToggleRequest(req, path, res) {
	...
	let warning = '';
	if (nextEnabled) {
		if (!cur.proxy && !ensureProxyForEnable()) {
		...
		// Precheck but never block: a dead proxy port still enables, with warning.
		warning = await precheckProxyForEnable();
	}
	const st = updateState((s) => {
		s.enabled = nextEnabled;
	});
	notifyState(st, warning);
	log(`toggle via ${path} -> ${st.enabled ? `ON ${st.proxy}` : 'OFF'}${warning ? ` (${warning})` : ''}`);
	writeJson(res, 200, warning ? { warning, ...statusSummary() } : statusSummary());
}
...
async function handleTestRequest(res) {
	...
	if (st.enabled && st.proxy && !(await probeProxyAlive(st.proxy))) {
		writeJson(res, 200, { ok: false, stage: 'proxy-unreachable', proxy: st.proxy, mode: st.mode, hint: '代理端口无响应，检查 VPN 客户端是否在运行' });
		return;
	}
	...
	if (exitIp === '') {
		writeJson(res, 200, { ok: false, stage: 'exit-probe-failed', via, proxy: st.proxy, mode: st.mode, hint: '两个出口端点都不可达（网络不通或端点被墙）' });
		return;
	}
...
<p id="info">加载中…</p>
<button id="t" onclick="testConn()">测试连通性</button>
<p id="tres"></p>
<p>每次请求即时生效 · 热键可在设置卡片启用</p>
</div><script>
...
async function testConn(){const t=document.getElementById('t'),o=document.getElementById('tres');t.disabled=true;o.textContent='测试中…';
 try{const r=await fetch('/vpn/test',{method:'POST'});const d=await r.json();
  o.textContent=d.ok?('出口 '+d.exitIp+' · '+d.latencyMs+'ms · '+(d.via==='proxy'?'经代理':'直连')):('测试未通过：'+(d.hint||d.stage||''));}catch(e){o.textContent='测试失败：'+e;}
 t.disabled=false;}
...
function paintState(s){if(s&&s.pill===false){if(pill.isConnected)pill.remove();return}if(s)mount();
if(s&&s.enabled)paint('VPN \\u25CF','#2f9e44','DSH 正走 VPN: '+s.proxy+'\\n点击关闭');else if(s)paint('VPN \\u25CB','#4b5563','DSH 直连\\n点击开启 VPN ('+(s.proxy||'未设置')+')');else paint('VPN \\u00D7','#6b7280','vpn-toggle 端点不可达（插件未加载？）')}
...
const apply = mountOnce('dsh-vpn-toggle', (ctx, config) => {
	...
	const requestToggle = async () => {
		...
		notifyState(st, warning);
		log(`hotkey toggle -> ${st.enabled ? `ON ${st.proxy}` : 'OFF'}${warning ? ` (${warning})` : ''}`);
	};
	...
	try {
		if (!existsSync(STATE_PATH)) {
			mkdirSync(dirname(STATE_PATH), { recursive: true });
			writeFileSync(STATE_PATH, JSON.stringify({ enabled: false, mode: 'all', proxy: '', noProxy: DEFAULT_NO_PROXY, allowProxy: '' }, null, 2) + '\n', 'utf8');
		}
	} catch (cause) {
