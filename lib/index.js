...
async function resolveProxyForEnable() {
	...
	if (!st.proxy) {
		if (!ensureProxyForEnable()) {
			...
			updateState((s) => {
				s.proxy = scanned;
				s.note = `未探测到系统代理，端口扫描命中 ${scanned}`;
			});
...
const apply = mountOnce('dsh-vpn-toggle', (ctx, config) => {
	...
	const sync = () => {
		...
		currentProxyConfigured = typeof value.proxy === 'string' && value.proxy.trim() !== '';
