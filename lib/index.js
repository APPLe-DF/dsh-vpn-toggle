...
import { normalizeProxyServer, shouldProxy, isValidProxyUrl, normalizeUserProxy, proxyFromScutilOutput } from './pure.js';
...
const HOST = HOST_ZH ? {
	...
	hintCandidateDead: 'VPN 未开启；所填代理端口无响应，检查客户端与端口',
	hintPending: '已通过该代理测得出口（VPN 未开启，打开开关后正式生效）',
	hintNoExitPending: '代理端口活着，但经它无法访问出口端点（该代理的出口不可用？）',
	hintAliveOnly: '代理端口存活（经代理的出口探测不可用）',
	hintDirectOff: 'VPN 未开启，测试的是直连出口'
} : {
	...
	hintCandidateDead: 'VPN is off; the configured proxy port is not responding - check client and port',
	hintPending: 'Exit reached through this proxy (VPN is off - flip the switch to apply it)',
	...
	hintDirectOff: 'VPN is off - this tests the direct exit'
};
 ...
 */
async function probeExit(doFetch) {
	const realFetch = doFetch || ((url, init) => fetch(url, init));
	const started = Date.now();
	for (const url of ['https://api.ipify.org/?format=json', 'https://ifconfig.me/ip']) {
		try {
			const r = await realFetch(url, { signal: AbortSignal.timeout(6000) });
			if (!r.ok) continue;
			let ip = '';
			if (url.indexOf('ipify') !== -1) ip = String(((await r.json()) || {}).ip || '');
			else ip = String(await r.text()).trim();
			if (ip) return { exitIp: ip, latencyMs: Date.now() - started };
		} catch {}
	}
	return null;
}
...
async function handleTestRequest(res) {
	...
	if (!st.enabled && st.proxy) {
		if (!(await probeProxyAlive(st.proxy))) {
			writeJson(res, 200, { ok: false, stage: 'proxy-unreachable', proxy: st.proxy, mode: st.mode, enabled: false, hint: HOST.hintCandidateDead });
			return;
		}
		let mod = null;
		try {
		...
		if (undici && undici.ProxyAgent && undici.fetch) {
			const agent = new undici.ProxyAgent({ uri: st.proxy });
			try {
				const result = await probeExit((url, init) => undici.fetch(url, Object.assign({}, init, { dispatcher: agent })));
				if (result) {
					writeJson(res, 200, { ok: true, exitIp: result.exitIp, latencyMs: result.latencyMs, via: 'proxy', pending: true, proxy: st.proxy, mode: st.mode, hint: HOST.hintPending });
				} else {
					writeJson(res, 200, { ok: false, stage: 'exit-probe-failed', via: 'proxy', proxy: st.proxy, mode: st.mode, enabled: false, hint: HOST.hintNoExitPending });
				}
			} catch (cause) {
				logErr('candidate proxy probe failed', cause);
				writeJson(res, 200, { ok: false, stage: 'candidate-probe-failed', proxy: st.proxy, mode: st.mode, enabled: false, hint: HOST.hintNoExitPending });
			} finally {
				try {
					agent.close();
				} catch {}
		...
		// undici unavailable (never seen in practice): report liveness only.
		writeJson(res, 200, { ok: false, stage: 'proxy-alive', proxy: st.proxy, mode: st.mode, enabled: false, hint: HOST.hintAliveOnly });
		return;
	}
	const result = await probeExit();
	if (!result) {
		writeJson(res, 200, { ok: false, stage: 'exit-probe-failed', via, proxy: st.proxy, mode: st.mode, hint: st.enabled ? HOST.hintNoExit : HOST.hintDirectOff });
		return;
	}
	const pending = !st.enabled;
	writeJson(res, 200, Object.assign({ ok: true, exitIp: result.exitIp, latencyMs: result.latencyMs, via: pending ? 'direct' : via, proxy: st.proxy, mode: st.mode }, pending ? { pending: true, hint: HOST.hintDirectOff } : {}));
}
...
const GUIDANCE = HOST_ZH ? [
	...
	'若开启后模型请求失败：先 POST /vpn/off 恢复直连，再检查 VPN 客户端与代理端口；换端口用 POST /vpn/proxy（body {"proxy":"http://127.0.0.1:端口"}，支持 http(s)://、socks5:// 与 host:port——自动补 http://）或设置卡片；代理留空时自动探测系统代理。',
	...
	'分流模式（POST /vpn/proxy 的 body 可带 {"mode":"all|allowlist","allowProxy":"a.com,.b.com"}）：all = 全部流量走 VPN；allowlist（白名单）= 仅命中 allowProxy 白名单的主机走 VPN（如「模型 API 直连 + web 抓取走 VPN」场景，可设 allowProxy 为抓取目标域名）。noProxy 优先级最高：命中绕过列表的目标即使命中 allowProxy 也直连。GET /vpn 返回的 mode/allowProxy 可原样汇报。'
].join('') : [
	...
	'If model requests fail after enabling: POST /vpn/off first to restore direct access, then check the VPN client and proxy port; change the port via POST /vpn/proxy (body {"proxy":"http://127.0.0.1:port"} - http(s)://, socks5:// and bare host:port are supported) or the settings card; with an empty address the system proxy is auto-detected.',
...
const Config = z.object({
	proxy: z.string().default('').description('VPN/代理软件在本机的入口地址（http(s)://、socks5:// 或 host:port）；留空 = 自动模式（自动探测系统代理，端口失效自动切换）'),
	noProxy: z.string().default(DEFAULT_NO_PROXY).description('永远直连、不走 VPN 的地址（逗号分隔，支持 .example.com 后缀与 *）；优先级最高'),
	mode: z.string().default('all').description('分流模式：all = 全部流量走 VPN；allowlist（白名单）= 仅白名单内的网站走 VPN，其余直连'),
	allowProxy: z.string().default('').description('代理白名单：仅白名单模式下走 VPN 的网站（逗号分隔，支持 .example.com 后缀）；留空 = 全部直连'),
	hotkey: z.string().default(DEFAULT_HOTKEY).description('全局热键：在任何界面按键即可开关 VPN（如 Control+Alt+V，可录制组合键）；留空不启用'),
	showPill: z.boolean().default(true).description('在 DSH 窗口右下角显示 VPN 开关小圆钮'),
	announceToAgent: z.boolean().default(true).description('让 AI 助手（agent）知道如何帮你开关 VPN')
});
...
const apply = mountOnce('dsh-vpn-toggle', (ctx, config) => {
	...
	const sync = () => {
		...
		const configured = typeof value.proxy === 'string' ? normalizeUserProxy(value.proxy) : '';
