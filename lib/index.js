...
async function resolveProxyForEnable() {
	...
	if (!st.proxy) {
		if (!ensureProxyForEnable()) {
			...
			return { switchedTo: scanned, note: `未探测到系统代理，端口扫描命中 ${scanned}` };
		}
		st = readState();
	}
	...
	if (detected && detected !== st.proxy && (await probeProxyAlive(detected))) {
		...
		return { switchedTo: detected, note: `已自动切换 → ${detected}` };
	}
	...
	if (scanned) {
		...
		return { switchedTo: scanned, note: `已自动切换 → ${scanned}（端口扫描）` };
	}
	return { warning: '代理端口无响应' };
}
...
const GUIDANCE = [
	...
	'开启时若配置的代理端口无响应：会自动重探系统代理（绕过缓存），仍不行再扫常见本地端口（7897/7890/10809/10808/2080/1080/8118），命中即自动切换并在通知与响应的 note 字段注明（如「已自动切换 → http://127.0.0.1:7890」）；全都不通才带「代理端口无响应」警告开启。',
