...
const Config = z.object({
	...
	mode: z.string().default('all').description('分流模式：all = 全部流量走 VPN；allowlist（白名单）= 仅命中 allowProxy 白名单的主机走 VPN'),
	allowProxy: z.string().default('').description('代理白名单（逗号分隔，支持 .example.com 后缀形式）；仅白名单（allowlist）模式生效，留空则全部直连'),
