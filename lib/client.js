window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const T = LANG_ZH ? {
			title: "VPN 开关",
			proxyLabel: "代理地址",
			proxyHint: "你的 VPN/代理软件在本机开出的入口。填 http(s)://… 或 socks5://…，也可以只填 127.0.0.1:7897（自动按 http 处理）；留空 = 自动模式：自动探测系统代理，端口没响应时还会自动换一个",
			proxyPlaceholder: "留空自动探测，如 http://127.0.0.1:7897 或 127.0.0.1:7897",
			invalidProxy: "代理地址必须是 http(s)://、socks5:// 或 host:port 形式",
			noProxyLabel: "绕过列表",
			noProxyHint: "永远不走 VPN、直接连接的地址（逗号分隔）。支持 .example.com 后缀与 * 通配；优先级最高，和白名单同时命中时以这里为准",
			modeLabel: "分流模式",
			modeHint: "全部流量 = DSH 的所有对外请求都走 VPN；仅白名单流量 = 只有白名单里的网站走 VPN，其余直连——适合「模型接口直连、网页抓取走 VPN」的用法",
			modeAll: "全部流量",
			modeAllowlist: "仅白名单流量",
			allowProxyLabel: "代理白名单",
			allowProxyHint: "只让这些网站走 VPN，其余照旧直连（逗号分隔，支持 .example.com 后缀）。仅在「仅白名单流量」模式下生效；留空 = 全部直连",
			allowProxyPlaceholder: "api.ipify.org,.github.com",
			hotkeyLabel: "全局热键",
			hotkeyHint: "在任何界面按下这个组合键即可一键开关 VPN。点「录制组合键」直接按键识别（Esc 取消），或手动填写（如 Control+Alt+V）；留空 = 不启用",
			...
			pillLabel: "悬浮按钮", pillHint: "在 DSH 窗口右下角显示一个 VPN 小圆钮，点击即可开关；关闭后 5 秒内自动消失",
			guidanceLabel: "agent 指引", guidanceHint: "让 AI 助手知道怎么帮你开关 VPN（对它说「开VPN」即可）",
			...
			pendingSuffix: "（VPN 未开启，仅预检代理）", proxyAlive: "代理端口存活",
			...
			proxyHint: "The local entry point of your VPN/proxy app. Accepts http(s)://, socks5://, or plain 127.0.0.1:7897 (http:// assumed). Leave empty for auto mode: the plugin detects the system proxy and switches ports automatically when one dies",
			...
			noProxyHint: "Addresses that never go through the VPN and always connect directly (comma separated). .example.com suffixes and * wildcards work; highest priority - wins over the whitelist",
			modeLabel: "Routing mode",
			modeHint: "All traffic = every DSH request goes through the VPN; Allowlist only = only whitelisted sites go through the VPN, everything else stays direct - good for model API direct + web scraping via VPN",
			modeAll: "All traffic",
			modeAllowlist: "Allowlist only",
			allowProxyLabel: "Proxy allowlist",
			allowProxyHint: "Only these sites go through the VPN, everything else stays direct (comma separated, .example.com suffixes work). Only effective in Allowlist-only mode; empty = nothing proxied",
			allowProxyPlaceholder: "api.ipify.org,.github.com",
			hotkeyLabel: "Global hotkey",
			hotkeyHint: "Press this combo anywhere to toggle the VPN. Click Record combo to capture it by pressing (Esc cancels), or type it (e.g. Control+Alt+V); empty = disabled",
			...
			pillLabel: "Floating pill", pillHint: "Show a small VPN button in the bottom-right corner of DSH; click it to toggle. It disappears within 5s when turned off",
			guidanceLabel: "Agent guidance", guidanceHint: "Teach the AI assistant how to toggle the VPN for you (just say: turn VPN on)",
			...
			pendingSuffix: " (VPN off - candidate proxy preview)", proxyAlive: "Proxy port alive",
			...
			inject() {
				return {
					...
					save: async () => {
						...
						// Bare host:port is accepted and stored canonicalized.
