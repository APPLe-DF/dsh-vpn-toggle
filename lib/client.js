window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const T = LANG_ZH ? {
			title: "VPN 开关",
			proxyLabel: "代理地址",
			proxyHint: "VPN 本地代理地址（http(s)://、socks5:// 或 host:port——自动补 http://）；留空 = 自动模式，插件自动探测并可在端口失效时自动切换",
			...
			modeLabel: "分流模式",
			modeHint: "全部流量 = 所有请求走 VPN；仅白名单流量 = 只有命中白名单的主机走 VPN（模型 API 直连 + web 抓取走 VPN）",
			modeAll: "全部流量",
			modeAllowlist: "仅白名单流量",
			allowProxyLabel: "代理白名单",
			allowProxyHint: "白名单模式下走 VPN 的主机（逗号分隔，支持 .example.com 后缀）；仅白名单模式生效，留空则全部直连",
			...
			proxyHint: "Local VPN proxy address (http(s)://, socks5:// or host:port - http:// assumed); leave empty for auto mode - the plugin detects the system proxy and can switch ports automatically when one dies",
