...
/**
 * Is this a storable proxy address? Only http(s):// and socks5:// URLs with
 * a hostname are accepted (the schemes the raw ProxyAgent is tested with);
 * anything else — javascript:, bare ports, garbage — must be rejected at
 * write time so a bad URI can never enter the dispatch pipeline.
 * Empty string is valid: it means auto mode.
 */
